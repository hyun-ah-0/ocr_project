# 💳 카드 명세서 OCR 기반 업무 자동화 서비스

카드 명세서 이미지를 자동으로 분석하여 거래 내역을 구조화하고,
카테고리 분류·지출 집계·요약 리포트를 생성하는 업무 자동화 서비스입니다.

수작업으로 명세서를 정리해야 하는 번거로움을 줄이고,
회계·경비 정산·개인 소비 관리 등 반복적인 문서 처리 업무를
AI 기반 OCR + LLM 파이프라인으로 자동화하는 것을 목표로 합니다.

---

## 📐 Architecture Diagram   

본 서비스는 카드 명세서 이미지 입력부터 거래 데이터 구조화,
카테고리 분류, 통계 요약까지 전 과정을 자동화하여
수작업 데이터 정리 업무를 최소화하도록 설계되었습니다.
```
[User] 
  ↓ (이미지 업로드)
[HTML/JS Frontend]
...
1️⃣ **카드 명세서 자동 데이터 추출**
  - OCR 기반 거래 정보 자동 인식
  - 날짜·금액·상호명 자동 구조화
  - 반복적인 수기 입력 업무 제거

2️⃣ **지출 데이터 자동 분류 및 정리**
  - 규칙 기반 + LLM 하이브리드 분류
  - 카테고리별 자동 집계
  - 정산 및 소비 분석 업무 지원
...
4️⃣ **다중 명세서 일괄 처리**
  - 여러 이미지 동시 분석(PNG, JPG, JPEG 형식 지원)
  - 누적 집계 자동화
  - 월별/기간별 소비 정리 가능

5️⃣ **고급 이미지 전처리 파이프라인**
   - Enhanced 전처리 (기본 사용)
     - 이미지 스케일링 (작은 이미지 확대)
     - 그레이스케일 변환
     - 샤프닝 필터 적용
